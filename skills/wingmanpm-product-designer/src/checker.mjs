...
function constructedDashMatches(content) {
  ...
  for (const match of content.matchAll(callPattern)) {
    const value = additiveInteger(match[2]);
    if (value === 0x2000 + 0x14 || value === 0x2000 + 0x13) matches.push({
      index: match.index,
...
function forbiddenDashMatches(content, extension) {
  const matches = [];
  const literals = [
    String['from' + 'CodePoint'](0x2000 + 0x13),
    String['from' + 'CodePoint'](0x2000 + 0x14)
  ];
  ...
  const patterns = [
    { regex: new RegExp(['&', '(?:m|n)dash(?:;|(?=[^0-9a-z]|$))'].join(''), 'gi'), kind: 'named HTML render equivalent' },
    { regex: new RegExp(['&#', '821[12](?:;|(?=[^0-9]|$))'].join(''), 'g'), kind: 'numeric HTML render equivalent' },
    { regex: new RegExp(['&#', 'x0*201[34](?:;|(?=[^0-9a-f]|$))'].join(''), 'gi'), kind: 'hex HTML render equivalent' }
  ];
  if (['.astro', '.cjs', '.js', '.jsx', '.json', '.mjs', '.svelte', '.ts', '.tsx', '.vue', '.yaml', '.yml'].includes(extension)) {
    patterns.push({
      regex: new RegExp('\\\\' + 'u(?:0{0,4}201[34]|\\{0*201[34]\\})', 'gi'),
  ...
  if (['.astro', '.cjs', '.css', '.html', '.js', '.jsx', '.mjs', '.scss', '.svelte', '.ts', '.tsx', '.vue'].includes(extension)) {
    patterns.push({
      regex: new RegExp('\\\\' + '0*201[34](?:\\s|(?=[^0-9a-f]|$))', 'gi'),
...
export async function runChecks(root, options = {}) {
  ...
  for (const contractFile of await tableContractFiles(root)) {
    ...
    if (validProjectPath(reviewTarget)) {
      ...
      if (!tableReviewDocument.exists) add(findings, root, 'block', 'WPD019', contractFile, `Declared table visual review is missing: ${reviewTarget}`);
        ...
        addValidationFindings(findings, root, 'WPD019', tableReviewFile, validateReview(tableReviewDocument.value));
        const reviewSeverity = options.allowPendingReview ? 'warn' : 'block';
        for (const check of REQUIRED_REVIEW_CHECKS) {
          if (tableReviewDocument.value.checks?.[check] !== true) add(findings, root, reviewSeverity, 'WPD019', tableReviewFile, `Visual review evidence for table ${contract.id} is missing ${check}.`);
        }
        ...
        if (tableReviewDocument.value.sourceHash !== currentHash) add(findings, root, reviewSeverity, 'WPD019', tableReviewFile, `Visual review evidence for table ${contract.id} is stale.`);
        const ageDays = (Date.now() - Date.parse(tableReviewDocument.value.reviewedAt ?? 0)) / 86_400_000;
        if (!Number.isFinite(ageDays) || ageDays > (config.visualEvidenceMaxAgeDays ?? 30)) add(findings, root, reviewSeverity, 'WPD019', tableReviewFile, `Visual review evidence for table ${contract.id} is too old.`);
      }
    }
  }
  ...
  if (!reviewDocument.error && review.status === 'pending') {
    ...
    const reviewSeverity = options.allowPendingReview ? 'warn' : 'block';
    for (const check of REQUIRED_REVIEW_CHECKS) {
      if (review.checks?.[check] !== true) add(findings, root, reviewSeverity, 'WPD011', reviewFile, `Visual review evidence is missing ${check}.`);
    }
    const viewports = new Set(review.viewports ?? []);
    for (const viewport of REQUIRED_VIEWPORTS) if (!viewports.has(viewport)) add(findings, root, reviewSeverity, 'WPD011', reviewFile, `Visual review is missing the ${viewport}px viewport.`);
    const sourceHash = await currentSourceHash();
    if (review.sourceHash !== sourceHash) add(findings, root, reviewSeverity, 'WPD011', reviewFile, 'Visual review evidence is stale because UI or design sources changed.');
    const ageDays = (Date.now() - Date.parse(review.reviewedAt ?? 0)) / 86_400_000;
    if (!Number.isFinite(ageDays) || ageDays > (config.visualEvidenceMaxAgeDays ?? 30)) add(findings, root, reviewSeverity, 'WPD011', reviewFile, 'Visual review evidence is too old.');
  }
