  ...
  try {
    ...
    if (await exists(detected.packageFile)) {
      ...
      const additions = {
        ...
        'design:doctor': `npx --yes wingmanpm-product-designer@${VERSION} doctor --project .`
      };
      if (detected.goldenStack) {
...
async function upgradeProject(flags) {
  ...
  if (!(await exists(root))) throw new Error(`Project does not exist: ${root}`);
  const manifestFile = path.join(root, PROJECT_MANIFEST);
  if (!(await exists(manifestFile))) throw new Error(`No WingmanPM project manifest in ${root}. Run npx --yes wingmanpm-product-designer@${VERSION} init first.`);
  let manifest;
...
async function doctorProject(flags) {
  ...
  if (manifest) {
    ...
    push('manifest-version', current ? 'pass' : 'fail', current
      ...
      : `Manifest is stale; run npx --yes wingmanpm-product-designer@${VERSION} upgrade to reach schemaVersion ${PROJECT_SCHEMA_VERSION} and version ${VERSION}.`);
  }
  for (const filename of ['AGENTS.md', 'CLAUDE.md']) {
    const target = path.join(root, filename);
  ...
  for (const asset of await runtimeAssets({ includePlaywright: await exists(path.join(root, 'playwright.wingman.config.ts')) })) {
    ...
    push(id, current ? 'pass' : 'fail', current
      ...
      : `${asset.relative} is missing or stale; run npx --yes wingmanpm-product-designer@${VERSION} upgrade with the current package.`);
  }
  ...
  push('table-inventory', inventoryError || (!savedInventory && discoveredTables.length) ? 'fail' : missingInventory.length || staleInventory.length ? 'warn' : 'pass',
    inventoryError
      ...
      ? `Table inventory is missing for ${discoveredTables.length} discovered surface(s); run npx --yes wingmanpm-product-designer@${VERSION} upgrade.`
  ...
  if (manifest && savedInventory) {
    ...
    push('table-inventory-ownership', inventoryEntry?.ownership === 'observed' ? 'pass' : 'fail', inventoryEntry?.ownership === 'observed'
      ...
      : `Table inventory ownership is stale; run npx --yes wingmanpm-product-designer@${VERSION} upgrade.`);
    ...
    push('table-contract-ownership', staleTableOwnership.length ? 'fail' : 'pass', staleTableOwnership.length
      ? `${staleTableOwnership.length} table contract entries are not user-owned; run npx --yes wingmanpm-product-designer@${VERSION} upgrade.`
  ...
  if (manifest) {
    ...
    push('browser-evidence-ownership', evidenceEntry?.ownership === 'observed' && evidenceEntry?.action === 'observed' ? 'pass' : 'fail',
      evidenceEntry?.ownership === 'observed' && evidenceEntry?.action === 'observed'
        ...
        : `Browser evidence ownership is stale; run npx --yes wingmanpm-product-designer@${VERSION} upgrade.`);
  }
  const report = await runChecks(root, { allowPendingReview: true });
  ...
  logJsonOrText(result, Boolean(flags.json), (value) => {
    ...
    if (value.kind === 'picker') {
      return [
        ...
        ...value.supportingReferences.map((reference) => `Supporting reference: ${reference}`)
      ].filter(Boolean).join('\n');
    }
    return [
