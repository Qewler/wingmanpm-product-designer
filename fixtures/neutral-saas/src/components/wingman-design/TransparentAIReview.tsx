  ...
  return (
    <section className="wpd-ai-review" aria-labelledby="ai-review-heading">
      <header>
        ...
        <div>
          <p className="wpd-eyebrow">AI draft · Human review required</p>
          <h2 id="ai-review-heading">Prepare a response</h2>
        </div>
      ...
      {status === 'draft' && (
        <>
          ...
          <footer>
            <p>Approval sends the reviewed draft. Nothing is sent automatically.</p>
            <div>
              <button className="wpd-secondary-button" type="button" onClick={onReject}><X aria-hidden="true" size={15} /> Reject</button>
              <button className="wpd-primary-button" type="button" onClick={onApprove}>Approve and send</button>
            </div>
          </footer>
        </>
      )}
