  ...
  return (
    <section aria-labelledby="workspace-heading">
      <div className="wpd-page-heading">
        <div>
          <p className="wpd-eyebrow">Current workspace {sample ? '· Sample data' : ''}</p>
          <h1 id="workspace-heading">Feedback operations</h1>
          <p>Review signals, resolve blockers, and keep the next action visible.</p>
        </div>
        <button className="wpd-primary-button" type="button">Create item</button>
      </div>
      ...
      <div className="wpd-toolbar" aria-label="Table controls">
        <label className="wpd-search-field">
          <span className="wpd-visually-hidden">Search work items</span>
          <Search aria-hidden="true" size={16} />
          <input type="search" placeholder="Search work items" />
        </label>
      ...
      <div className="wpd-table-region" tabIndex={0} aria-label="Scrollable work items table">
        <table>
          <caption className="wpd-visually-hidden">Work items and their current status</caption>
          <thead>
            <tr>
